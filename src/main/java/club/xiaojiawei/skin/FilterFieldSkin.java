...
public class FilterFieldSkin extends IconTextFieldSkin {
    ...
    public FilterFieldSkin(FilterField filterField) {
        ...
        if (button.getGraphic() instanceof SVGPath svgPath){
            button.setOnMouseEntered(event -> {
                FillTransition fillTransition = new FillTransition(Duration.millis(250), svgPath);
                fillTransition.setToValue(Color.valueOf("#0075FF"));
                fillTransition.play();
            });
            button.setOnMouseExited(event -> {
                FillTransition fillTransition = new FillTransition(Duration.millis(250), svgPath);
                fillTransition.setToValue(Color.valueOf("#4E5053FF"));
                fillTransition.play();
            });
        }
